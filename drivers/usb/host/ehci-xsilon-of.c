 ...
 *
 * Bus Glue for Xsilon EHCI core on the of_platform bus
 *
...
static const struct hc_driver ehci_xsilon_of_hc_driver = {
...
/**
 * ehci_hcd_xsilon_of_probe - Probe method for the USB host controller
 ...
 */
static int ehci_hcd_xsilon_of_probe(struct platform_device *op)
{
	...
	hcd = usb_create_hcd(&ehci_xsilon_of_hc_driver, &op->dev,
				"XILINX-OF USB");
	if (!hcd)
		return -ENOMEM;
...
/**
 * ehci_hcd_xsilon_of_remove - shutdown hcd and release resources
 * @op:		pointer to platform_device structure that is to be removed
 *
 * Remove the hcd structure, and release resources that has been requested
 * during probe.
 */
static int ehci_hcd_xsilon_of_remove(struct platform_device *op)
{
	struct usb_hcd *hcd = platform_get_drvdata(op);

	dev_dbg(&op->dev, "stopping XSILON-OF USB Controller\n");
...
static const struct of_device_id ehci_hcd_xsilon_of_match[] = {
		{.compatible = "xlnx,usba-controller-1.00.a",},
	{},
};
MODULE_DEVICE_TABLE(of, ehci_hcd_xsilon_of_match);

static struct platform_driver ehci_hcd_xsilon_of_driver = {
	.probe		= ehci_hcd_xsilon_of_probe,
	.remove		= ehci_hcd_xsilon_of_remove,
	.shutdown	= usb_hcd_platform_shutdown,
	.driver = {
		.name = "xsilon-of-ehci",
		.owner = THIS_MODULE,
		.of_match_table = ehci_hcd_xsilon_of_match,
	},
};
